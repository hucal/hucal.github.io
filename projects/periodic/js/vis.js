...
d3.select(document).on('DOMContentLoaded', function() {
  d3.json('table.json', function(error, elements) {
...
mk_periodic_table = function() {
  ...
  tip = d3.tip().attr('class', 'd3-tip').html(function(d) {
    var j, len, name, prop, ref, s;
    s = "<em>" + d.name + " (" + d.sym + ")</em><table>";
    for (j = 0, len = prettyNames.length; j < len; j++) {
      ref = prettyNames[j], name = ref[0], prop = ref[1];
  ...
  draw_controls = function() {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      _class = _class[0];
      cc.append('span').append('select').attr('onchange', "vis." + _class + "(this.value)").selectAll('option').data(Object.keys(classifiers).concat([""])).enter().append('option').each(function(d) {
    ...
    for (l = 0, len1 = ref1.length; l < len1; l++) {
      ...
      cc.append('input').attr('type', 'checkbox').attr('aria-labelledby', "toggle_" + x).attr('id', "toggle_" + x + "_checkbox").attr('checked', x === 'zoom' ? null : 'checked').on('change', (function(x) {
        return function() {
