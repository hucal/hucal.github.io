function mk_hp_force() {
    var nodes = 20, links = 40, draw_nodes = true, draw_force = true,
        radius_by_what = "by_b",
        axis_by_what = "by_a";
    var svg;
...
var links = random_links(nodes, config.links, config.links, randint);
...
var angle = d3.scale.ordinal().domain(d3.range(0,3)).rangePoints([0, 4/3 * Math.PI])
    ...
    color = d3.scale.ordinal().domain(d3.range(3)).range(colorbrewer.RdYlGn[4]);
...
svg.selectAll("text.axis_title")
    .data(angle.range())
  .enter().append("text")
    .attr("class", "axis_title")
    .attr("x", function (d) { return outerRadius * Math.cos(d3.mean(d)); })
    .attr("y", function (d) { return outerRadius * Math.sin(d3.mean(d)); })
